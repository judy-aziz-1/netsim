...
describe('attack role guards', () => {
  ...
  it('succeeds when the attacker is wired directly to the victim with no intermediate device (regression)', () => {
    // Previously, a direct wire meant getImmediateNeighborId(victim, links) resolved
    // to the attacker's own id, and isAttackerOnSameSegment then looked for a
    // (nonsensical) self-link and always failed — rejecting the simplest possible
    // adjacency case. Fixed in isAttackerOnSameSegment (pingRouting.js).
    const { addDevice, addLink, triggerArpSpoof } = useTopologyStore.getState();
