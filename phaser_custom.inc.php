...
<body>
    ...
    <script src="editor/js/ui/siem.js"></script>
    ...
    <script src="editor/levels/routing.js"></script>
    <!-- Config -->
    ...
    <div id="topbar">
        ...
        <div id="topbar-brand">
            NETSIM
            <?php if (LOGGEDIN && isset($db)):
                $uq = $db->prepare("SELECT name FROM user WHERE id = :id");
                $uq->bindValue(':id', $_SESSION['cs4g_user_id']);
                $ur = $uq->execute()->fetchArray();
            ?>
                <span class="topbar-user">| 👤 <?= htmlspecialchars($ur['name'] ?? '') ?></span>
            <?php endif; ?>
        </div>
    </div>
    ...
    <!-- SIEM Panel -->
    <div id="siem-panel" class="collapsed">
        <div id="siem-header">
            <div id="siem-title">SIEM MONITOR</div>
            <div id="siem-controls">
                <span id="siem-counter">Events: <span id="siem-count">0</span></span>
                <button class="siem-btn" id="siem-clear-btn">Clear</button>
                <button id="siem-toggle-btn">▲</button>
            </div>
        </div>
        <div id="siem-body">
            <div id="siem-empty">No events yet — run a Ping to start monitoring</div>
            <table id="siem-table" style="display:none">
                <thead>
                    <tr>
                        <th>Time</th>
                        <th>Source IP</th>
