...
function generateSmartIP(typeKey) {
    // Router يأخذ IP عشوائي عادي
    if (typeKey === 'router') return generateIP();

    // غير Router — ابحث عن Router موجود وأعطِ IP من شبكته
    var routers = devices.filter(function(d) {
        return d.deviceData && d.deviceData.role === 'router';
    });

    if (routers.length === 0) return generateIP();

    // خذ أول Router وأعطِ IP في نفس الشبكة
    var routerIP = routers[0].deviceData.ip;
    var parts    = routerIP.split('.');
    // نفس الـ x.x.x مع host عشوائي مختلف
    var usedHosts = devices.map(function(d) {
        return d.deviceData ? parseInt(d.deviceData.ip.split('.')[3]) : 0;
    });
    var host;
    do { host = Math.floor(Math.random() * 200) + 10; }
    while (usedHosts.indexOf(host) !== -1 || host === parseInt(parts[3]));
...
function createDeviceSprite(scene, x, y, typeKey) {
   ...
   sprite.deviceData = {
    ...
    if (roleTints[typeKey]) sprite.tint = roleTints[typeKey];
...
function showEditPanel(sprite) {
    ...
    panel.innerHTML =
        ...
        '<label style="' + labelStyle + '">IP Address' + getSuggestedNetworkHint(sprite) + '</label>' +
...
[nameInput, ipInput, gwInput].forEach(function(inp) {
    ...
    document.getElementById('edit-save-btn').addEventListener('click', function() {
        ...
        // تحديث اللون عند تغيير الـ Role
        var roleTints = {
            'pc':       0xffffff,
            'router':   0x99ccff,
            'switch':   0xc084fc,
        ...
        if (roleTints[d.role]) sprite.tint = roleTints[d.role];
 ...
 function closePanel() {
    ...
    function getSuggestedNetworkHint(sprite) {
    // ابحث عن Router متصل مباشرة بهذا الجهاز
    var connectedRouter = null;
    links.forEach(function(L) {
        if (L.enabled === false) return;
        var other = null;
        if (L.a === sprite) other = L.b;
        else if (L.b === sprite) other = L.a;
        if (other && other.deviceData.role === 'router') connectedRouter = other;
    });

    if (!connectedRouter) return '';

    var rIP  = connectedRouter.deviceData.ip;
    var net  = rIP.split('.').slice(0, 3).join('.');
    return ' <span style="color:#4d9fff;font-size:8px;margin-right:4px">← suggested: ' + net + '.x</span>';
}
