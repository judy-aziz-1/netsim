  ...
  function linkExists(a, b) {
    ...
    function addLink(a, b) {
       ...
       g.lineStyle(2, 0x94a3b8, 1);
        ...
        // Tooltip عند hover
        hit.events.onInputOver.add(function(h) {
            showLinkTooltip(h.linkRef);
        });
        ...
        // ===== اقتراح IP عند الوصل بـ Router =====
        suggestIPOnLink(a, b);
    }

    /* ===== اقتراح IP عند الوصل بـ Router ===== */
function suggestIPOnLink(a, b) {
    var router   = null;
    var endpoint = null;

    if (a.deviceData.role === 'router' && b.deviceData.role !== 'router') {
        router = a; endpoint = b;
    } else if (b.deviceData.role === 'router' && a.deviceData.role !== 'router') {
        router = b; endpoint = a;
    }

    if (!router || !endpoint) return;

    var routerNet   = getNetworkAddress(router.deviceData.ip,   router.deviceData.subnet   || '255.255.255.0');
    var endpointNet = getNetworkAddress(endpoint.deviceData.ip, endpoint.deviceData.subnet || '255.255.255.0');

    // إذا بالفعل على نفس الشبكة — لا حاجة لاقتراح
    if (routerNet === endpointNet) return;

    // اقترح IP من شبكة الـ Router
    var rParts   = router.deviceData.ip.split('.');
    var usedLast = devices.map(function(d) {
        var p = d.deviceData.ip.split('.');
        if (p[0]===rParts[0] && p[1]===rParts[1] && p[2]===rParts[2])
            return parseInt(p[3]);
        return -1;
    });
    var host;
    do { host = Math.floor(Math.random() * 200) + 10; }
    while (usedLast.indexOf(host) !== -1 || host === parseInt(rParts[3]));

    var suggestedIP = rParts[0]+'.'+rParts[1]+'.'+rParts[2]+'.'+host;

    // أظهر toast مع زر قبول
    showIPSuggestionToast(endpoint, suggestedIP, routerNet, router.deviceData.name);
}

/* ===== Toast اقتراح الـ IP ===== */
function showIPSuggestionToast(device, suggestedIP, network, routerName) {
    var old = document.getElementById('ip-suggestion-toast');
    if (old) old.remove();

    var box = document.createElement('div');
    box.id = 'ip-suggestion-toast';
    box.style.cssText = [
        'position:fixed','bottom:60px','left:50%','transform:translateX(-50%)',
        'background:#161b22','border:1px solid rgba(77,159,255,0.4)',
        'border-radius:8px','padding:12px 18px','z-index:999999',
    ...
    box.innerHTML =
        '<span style="color:#4d9fff;font-size:10px">💡</span>' +
        '<span style="color:#7d8590">' + device.deviceData.name + ' على شبكة مختلفة عن ' + routerName + '</span>' +
        '<span style="color:#484f58">|</span>' +
        '<span style="color:#4d9fff">اقتراح: ' + suggestedIP + '</span>' +
        '<button id="ip-suggest-accept" style="background:rgba(77,159,255,0.15);color:#4d9fff;border:1px solid rgba(77,159,255,0.4);border-radius:5px;padding:4px 12px;font-family:JetBrains Mono,monospace;font-size:10px;cursor:pointer;font-weight:600">تطبيق</button>' +
        '<button id="ip-suggest-ignore" style="background:transparent;color:#484f58;border:none;cursor:pointer;font-size:13px">✕</button>';

    document.body.appendChild(box);

    document.getElementById('ip-suggest-accept').addEventListener('click', function() {
        device.deviceData.ip = suggestedIP;
        if (typeof logEvent === 'function')
            logEvent('success', '✅ IP updated: ' + device.deviceData.name + ' → ' + suggestedIP);
        if (typeof showToast === 'function')
            showToast(device.deviceData.name + ' → ' + suggestedIP, 'success');
        box.remove();
    });

    document.getElementById('ip-suggest-ignore').addEventListener('click', function() {
        box.remove();
    });
...
/* ===== Link Tooltip ===== */
function showLinkTooltip(linkObj) {
    hideLinkTooltip();

    var rect = document.getElementById('editorCanvas').getBoundingClientRect();
    var midX = rect.left + (linkObj.a.x + linkObj.b.x) / 2;
    var midY = rect.top  + (linkObj.a.y + linkObj.b.y) / 2 - 50;

    var isEnabled = linkObj.enabled !== false;
    var statusColor = isEnabled ? '#30c896' : '#ff5f5f';
    ...
    tip.innerHTML =
        ...
        '<span style="color:#e6edf3">' +
            linkObj.a.deviceData.name +
            ' <span style="color:#484f58">↔</span> ' +
            linkObj.b.deviceData.name +
        '</span>';
...
function hideLinkTooltip() {
    var old = document.getElementById('link-tooltip');
    if (old) old.remove();
}
